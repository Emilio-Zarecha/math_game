...
**Textbook switcher nav bar** (`textbook-switcher.js`) — injected dynamically after the first `<header>` element on the page (works on any page that has one — every textbook, plus `index.html` and `diagrams.html`, each of which wraps its title in `<header>` specifically so this script has an anchor point). Renders as a horizontal carousel (single row, `overflow-x: auto`, hidden scrollbar, edge-fade `mask-image` since the scrollbar itself is hidden) inside a `.textbook-switcher-wrap` flex row, flanked by `‹`/`›` `.ts-scroll-btn` buttons that call `nav.scrollBy({left: ±160, behavior:'smooth'})`. The bar overflows at every practical viewport width (13 items, ~1300px of content vs. a ~760–920px content column), so the buttons are the primary way to reach items off-screen — each auto-hides (`disabled` → `opacity:0; pointer-events:none`) once its direction has nothing left to scroll. An `EXTRAS` array prepends "◂ Game" and "Diagrams" entries (visually separated by a border) before the `BOOKS` list, so this bar doubles as the site's one persistent nav, not just a textbook-to-textbook switcher. Active page marked with `class="current"` and `aria-current="page"`.

**CRITICAL — page-specific width overrides must target `.textbook-switcher-wrap`, not `nav.textbook-switcher`.** `index.html` constrains the bar to `#layout`'s max-width via a page-specific CSS rule; before the buttons were added this targeted `nav.textbook-switcher` directly (the only element there). Now that `nav` is nested inside `.textbook-switcher-wrap` alongside the two buttons, a rule that still targets the inner `nav` leaves the outer wrap with no defined width — since the wrap is a flex item of `body` (`align-items:center`, no stretch) with no width of its own, its auto-sizing can expand to the nav's max-content size regardless of viewport, pushing the whole bar off-screen. Always size the outer `.textbook-switcher-wrap`, and let the inner `nav`'s `flex:1; min-width:0` handle shrinking to fit inside it.
...
**Button placement:** `theme-toggle.js` looks for a `<header>` element and appends the button there with `margin-left:auto` — this requires `header { display:flex; ... }` on that page (every textbook has this; `index.html` and `manual.html` do too, specifically so this works). If no `<header>` is found, it falls back to a fixed position at top-right. Every textbook now has a `<header>` — `textbook_linearalgebra.html` and `textbook_statistics.html` used the older bare `<a class="back-link">` + `<h1>` pattern (no `<header>`) until it was fixed, which had silently disabled `textbook-switcher.js` on those two pages too (see the switcher's `<header>` dependency above); keep new textbooks wrapping their title/back-links in `<header>` from the start.
...
- **New pages must use `<header>` for title/back-links, not a bare `<h1>`**: `textbook-switcher.js` and `theme-toggle.js` both locate their anchor via `document.querySelector('header')` and silently no-op if it's missing. `textbook_linearalgebra.html` and `textbook_statistics.html` used an older bare-`<h1>` pattern and had no switcher bar or theme toggle at all until this was caught by auditing every page headlessly rather than trusting one visual spot-check.
- **Verify UI changes headlessly if the browser extension isn't connected**: when `claude-in-chrome` won't connect, `npm install puppeteer` in the scratchpad and drive a headless Chromium against the local dev server instead of skipping visual verification — it caught both the `<header>`-less pages above and the `.textbook-switcher-wrap` width regression (see the switcher section) that a code-only review would have missed.
